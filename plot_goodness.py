...
# xtick_vals = np.arange(0, 301, 50)
# ytick_vals = np.linspace(1.0, 5.0, 9)
# xtick_vals_str = xtick_vals.astype('str')
# ytick_vals_str = ytick_vals.astype('str')

ax.set_xlabel('$\ell$', fontsize=18)
ax.set_ylabel('Unperturbed frequency $\\nu_0$ in mHz',fontsize=18)
ax.tick_params("both", labelsize=14)
# ax.set_xticklabels(xtick_vals_str, fontsize=16)
# ax.set_yticklabels(ytick_vals_str, fontsize=16)
# plt.tight_layout()
fig.subplots_adjust(left=0.080, right=1.05, bottom=0.15, top=0.85)
fig.savefig(f'{datadir}/Cross_coupling_goodness/DR_all.pdf')
plt.close(fig)
