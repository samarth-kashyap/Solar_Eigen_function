...
nl_all = np.loadtxt(f'{datadir}/nl.dat')
...
for i in range(len(nl_list)):
    ...
    for j in range(len(nl_list)):
        ...
        if(os.path.exists(f'{datadir}/DR_Simulation_submatrices/{n_}_{l_}_{n}_{l}.npy')):
            print('Computed mode exists')
            Z_large[mi_beg:mi_end,mj_beg:mj_end] = np.load(f'{datadir}/DR_Simulation_submatrices/{n_}_{l_}_{n}_{l}.npy')

        #Checking if the flipped modes have been calculated.
        elif(os.path.exists(f'{datadir}/DR_Simulation_submatrices/{n}_{l}_{n_}_{l_}.npy')):
            print('Computed flipped mode exists')
            Z_CT = np.load(f'{datadir}/DR_Simulation_submatrices/{n}_{l}_{n_}_{l_}.npy')
    ...
    #        temp_matrix = np.diag(submatrix.submatrix_diagonal_diffrot(n_,l_,r,omega_nl[i]))
            ...
            np.save(f'{datadir}/DR_Simulation_submatrices/{n_}_{l_}_{n}_{l}.npy',Z_large[mi_beg:mi_end,mj_beg:mj_end])                       
...
fname_qdpt = f"{datadir}/new_freqs/qdpt_{args.n0:02d}_{args.l0:03d}.npy"
fname_dpt = f"{datadir}/new_freqs/dpt_{args.n0:02d}_{args.l0:03d}.npy"
np.save(fname_qdpt, f_qdpt_arranged[:2*l0+1])
np.save(fname_dpt, f_dpt[:2*l0+1])

# saving each offset multiplet in a different file
np.savetxt(f"{datadir}/Cross_coupling_goodness/offsets_{n0:02d}_{l0:03d}.dat",
...
plt.tight_layout()
plt.savefig(f'{datadir}/Cross_coupling_goodness/DR_all.pdf')
